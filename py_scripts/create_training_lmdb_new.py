...
def parse_chain_atoms(pdb_file, chain_id):
    """从PDB文件中解析指定链的原子信息"""
    ...
    atoms = []
    coordinates = []
    residues_info = []  # 存储残基信息用于pocket提取
...
def extract_pocket_residues(receptor_atoms, receptor_coords, receptor_residues, 
                          peptide_coords, cutoff=5.0):
    """提取与peptide相互作用的receptor残基（pocket）"""
    if len(peptide_coords) == 0 or len(receptor_coords) == 0:
        return [], np.array([]), []
    
    # 计算peptide和receptor原子之间的距离
    distances = cdist(peptide_coords, receptor_coords)
    
    # 找到距离小于cutoff的receptor原子
    interacting_receptor_indices = np.where(distances.min(axis=0) < cutoff)[0]
    
    # 获取相互作用的残基ID
    interacting_residue_ids = set()
    for idx in interacting_receptor_indices:
        for res_info in receptor_residues:
            if res_info['atom_index'] == idx:
                interacting_residue_ids.add(res_info['residue_id'])
                break
    ...
    for i, res_info in enumerate(receptor_residues):
        if res_info['residue_id'] in interacting_residue_ids:
            pocket_atoms.append(receptor_atoms[res_info['atom_index']])
            pocket_coords.append(receptor_coords[res_info['atom_index']])
    
    return pocket_atoms, np.array(pocket_coords), list(interacting_residue_ids)
...
def save_peptide_pdb(pdb_file, peptide_chain, output_dir):
    """保存peptide链为单独的PDB文件"""
    parser = PDB.PDBParser(QUIET=True)
    structure = parser.get_structure('protein', pdb_file)
    ...
    output_filename = f"{os.path.splitext(os.path.basename(pdb_file))[0]}_{peptide_chain}_peptide.pdb"
    output_path = os.path.join(output_dir, output_filename)
    
    io.save(output_path, PeptideSelector(peptide_chain))
    return output_path
...
def save_complex_pdb(pdb_file, peptide_chain, receptor_chain, pocket_residue_ids, output_dir):
    """保存peptide和pocket合并的复合体PDB文件"""
    parser = PDB.PDBParser(QUIET=True)
    structure = parser.get_structure('protein', pdb_file)
    ...
    for model in structure:
        for chain in model:
            if chain.id == peptide_chain:
                ...
                new_chain.id = 'P'  # 重命名为P链表示peptide
    ...
    output_filename = f"{os.path.splitext(os.path.basename(pdb_file))[0]}_complex.pdb"
    output_path = os.path.join(output_dir, output_filename)
    
    io.save(output_path)
    return output_path

def process_pdb_file(pdb_file, cutoff=5.0, peptide_dir=None, pocket_dir=None, complex_dir=None):
    """处理单个PDB文件，提取peptide和对应的pocket"""
    try:
        # 自动判断哪个链是peptide，哪个是receptor
        peptide_chain, receptor_chain = determine_peptide_receptor_chains(pdb_file)
        
        # 解析peptide链
        peptide_atoms, peptide_coords, _ = parse_chain_atoms(pdb_file, peptide_chain)
        
        # 解析receptor链
        receptor_atoms, receptor_coords, receptor_residues = parse_chain_atoms(
        ...
        # 提取与peptide相互作用的pocket
        pocket_atoms, pocket_coords, pocket_residue_ids = extract_pocket_residues(
            receptor_atoms, receptor_coords, receptor_residues, 
            peptide_coords, cutoff)
        
        if len(pocket_atoms) == 0:
            print(f"Warning: No pocket residues found for {pdb_file}")
            return None
        
        # 保存peptide和pocket为单独的PDB文件
        peptide_pdb_path = None
        pocket_pdb_path = None
        complex_pdb_path = None
        
        if peptide_dir is not None:
            peptide_pdb_path = save_peptide_pdb(pdb_file, peptide_chain, peptide_dir)
        
        if pocket_dir is not None:
            pocket_pdb_path = save_pocket_pdb(pdb_file, receptor_chain, pocket_residue_ids, pocket_dir)
        
        if complex_dir is not None:
            complex_pdb_path = save_complex_pdb(pdb_file, peptide_chain, receptor_chain, pocket_residue_ids, complex_dir)
        
        # 构建数据条目
        data_entry = {
            'pocket1': f"{os.path.basename(pdb_file)}_{peptide_chain}_peptide",
            'pocket1_atoms': peptide_atoms,
            'pocket1_coordinates': peptide_coords.astype(np.float32),
            ...
            'label': 1,  # peptide和其对应的pocket，认为是匹配的
        }
...
def split_train_valid(pdb_files, valid_ratio=0.2, seed=42):
    """划分训练集和验证集"""
    random.seed(seed)
    np.random.seed(seed)
    
    # 打乱文件列表
    pdb_files_copy = pdb_files.copy()
    random.shuffle(pdb_files_copy)
    
    # 计算验证集大小
    valid_size = int(len(pdb_files_copy) * valid_ratio)
    
    # 划分
    valid_files = pdb_files_copy[:valid_size]
    train_files = pdb_files_copy[valid_size:]
    
    print(f"Total files: {len(pdb_files_copy)}")
    print(f"Training files: {len(train_files)}")
    print(f"Validation files: {len(valid_files)}")
    
    return train_files, valid_files

def create_lmdb_from_file_list(file_list, pdb_dir, output_lmdb, cutoff, peptide_dir=None, pocket_dir=None, complex_dir=None, dataset_type="train"):
    """从文件列表创建LMDB"""
    # 创建输出目录
    if peptide_dir is not None:
    ...
    with env.begin(write=True) as txn:
        ...
        for pdb_file in tqdm(file_list, desc=f"Creating {dataset_type} {os.path.basename(output_lmdb)}"):
            ...
            data_entry = process_pdb_file(pdb_path, cutoff, peptide_output_dir, pocket_output_dir, complex_output_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--valid_ratio", type=float, default=0.2, help="Ratio of validation set (0.0-1.0)")
    ...
    # 划分训练集和验证集
    train_files, valid_files = split_train_valid(pdb_files, args.valid_ratio, args.seed)
    
    # 定义输出文件路径
    train_lmdb = os.path.join(args.output_dir, "train.lmdb")
    valid_lmdb = os.path.join(args.output_dir, "valid.lmdb")
    
    # 创建训练集
    create_lmdb_from_file_list(train_files, args.pdb_dir, train_lmdb, 
                             args.cutoff, args.peptide_dir, args.pocket_dir, args.complex_dir, "train")
    
    # 创建验证集
    create_lmdb_from_file_list(valid_files, args.pdb_dir, valid_lmdb, 
                             args.cutoff, args.peptide_dir, args.pocket_dir, args.complex_dir, "valid")
    
    print("Conversion completed!")
    print(f"Training set: {train_lmdb}")
    print(f"Validation set: {valid_lmdb}")